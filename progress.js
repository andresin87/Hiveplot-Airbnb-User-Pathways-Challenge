...
(function (root, factory) {
  ...
  function _setProgress(targetElement) {
	...
	//create a centered wrapped element of progress bar
	var progressCenteredElement = document.createElement('div');
    progressCenteredElement.className = "progressjs-centered";
    
    //create an element for Airbmb Logo
    var inner = document.createElement('div');
    inner.className = "progressjs-airbnb-logo";
    inner.style.marginBottom = "-30px";
    inner.innerHTML = "<img src='airbnb_white.svg'></img>";
    progressCenteredElement.appendChild(inner);
    
    //create an element for Page Status
    if(this._options.status!=""){
	    var pStatus = document.createElement('div');
	    pStatus.className = "progressjs-page-status";
	    pStatus.style.marginBottom = "-30px";
	    pStatus.innerHTML = this._options.status;
	    progressCenteredElement.appendChild(pStatus);
    }
    ...
    progressCenteredElement.appendChild(progressPercentElement);
       
    progressElement.appendChild(progressCenteredElement);
  ...
  progressJs.fn = ProgressJs.prototype = {
    ...
    setStatus: function(stat){
	    this._options.status = stat;
	    targetElement = this._targetElement[0];
	    if (targetElement.hasAttribute("data-progressjs")) {
		    var statusElement = _getPercentElement(targetElement);
	
	        var statusElement  = statusElement.querySelector(".progressjs-page-status");
	        if(statusElement)
	        	statusElement.innerHTML = this._options.status;
	        else{
			    //create an element for Page Status
			    if(this._options.status!=""){
				    targetElement = targetElement.querySelector(".progressjs-centered");
				    nextSibling = targetElement.querySelector(".progressjs-percent");
				    
				    var pStatus = document.createElement('div');
				    pStatus.className = "progressjs-page-status";
				    pStatus.style.marginBottom = "-30px";
				    pStatus.innerHTML = this._options.status;
				    targetElement.insertBefore(pStatus, nextSibling);
			    }
			}
	    }
	    return this;
